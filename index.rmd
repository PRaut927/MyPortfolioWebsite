...
This is a short tutorial I made from the video tutorial on the dplyr package by Hadley Wickham. The link for the video tutorial:<br /> 
...
flights %>%
  ...
  group_by(dest) %>%
  summarize(Average_delay = mean(arr_delay), cases = n()) %>%
  filter(cases > 10) %>%
...
flights %>%
  ...
  summarise(n = n_distinct(time_hour)) %>%
  filter(n == 365)
```
...
flights %>%
  ...
  summarize(n = n(), mean = mean(na.omit(dep_delay)))
```
